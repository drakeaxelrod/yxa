//! SZR35 Keyboard Layout Trainer
//!
//! A training tool for learning keyboard layouts with real-time layer visualization.
...
#[derive(Parser)]
#[command(name = "szr35-trainer")]
#[command(about = "SZR35 keyboard layout trainer")]
#[command(version)]
struct Cli {
...
fn find_layout_file(specified: Option<PathBuf>) -> Result<PathBuf> {
    ...
    if let Some(config_dir) = dirs::config_dir() {
        let path = config_dir.join("szr35/layout.vil");
        if path.exists() {
            return Ok(path);
        }
    }
